...
// ✅ NEW: Fetch user availability when start/end times change in edit modal
useEffect(() => {
if (!showEditModal || !formData.start || !formData.end) return;
// Convert Brisbane local time to UTC for API call
const startUTC = toUTCISOString(formData.start);
const endUTC = toUTCISOString(formData.end);
console.log('🕒 Fetching availability for edit:', { 
localStart: formData.start, 
localEnd: formData.end,
utcStart: startUTC,
utcEnd: endUTC
});
fetch(`/api/users/availability?start=${startUTC}&end=${endUTC}`)
.then(res => res.json())
.then((data: UserAvailability[]) => {
